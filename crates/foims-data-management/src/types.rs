...
/// 成功 JSON 响应构造（由 foims-common 提供，保持原有路径兼容）。
...
impl From<validator::ValidationErrors> for DataError {
    fn from(err: validator::ValidationErrors) -> Self {
        DataError::Validation(foims_common::validation_errors_to_message(&err))
    }
}
...
impl From<DataError> for foims_common::AppError {
    fn from(err: DataError) -> Self {
        match err {
            DataError::Database(m) => foims_common::AppError::Database(m),
            DataError::NotFound(m) => foims_common::AppError::NotFound(m),
            DataError::Validation(m) => foims_common::AppError::Validation(m),
            DataError::Conflict(m) => foims_common::AppError::Conflict(m),
            DataError::Internal(m) => foims_common::AppError::Internal(m),
        }
    }
}
