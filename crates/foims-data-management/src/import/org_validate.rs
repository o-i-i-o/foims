//! 组织模板/组织导入行的结构校验（与 API 端 `foims-organization` 的
//! `validate_levels_mapping`、`resolve_type_name` 语义对齐）。
//!
//! CSV 导入路径无法直接依赖 foims-organization（依赖方向不允许），
...
pub fn validate_levels_mapping(levels: &Value) -> Result<(), foims_common::AppMessage> {
