...
pub async fn create_database_api(
    ...
    foims_common::log_info!("log.init.db.database_created");
    Ok(ok_json(
...
/// 作用域结束时删除落盘文件，避免 /tmp/foims_import 无限累积
struct TempSqlFile {
    path: PathBuf,
}
...
impl Drop for TempSqlFile {
    fn drop(&mut self) {
        ...
        {
            foims_common::log_warn!("log.init.db.temp_file_remove_failed", error = e);
        }
    }
}
...
pub async fn import_database_from_file(
    ...
    tokio::fs::create_dir_all("/tmp/foims_import")
        .await
        .map_err(|e| {
    ...
    while let Some(mut field) = payload.next_field().await.map_err(|e| {
        ...
        if field_name == "verification" {
            ...
            let filepath =
                PathBuf::from(format!("/tmp/foims_import/import_{}.sql", Uuid::new_v4()));
    ...
    foims_common::log_info!("log.init.db.imported_from_file");
    Ok(ok_json(
        CreateDatabaseResponse {
            backup_file,
...
pub async fn clear_database(
    ...
    foims_common::log_info!("log.init.db.clearing");
    ...
    foims_common::log_info!("log.init.db.cleared");
