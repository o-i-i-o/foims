...
use dashmap::DashMap;
use foims_common::config::{Config, parse_duration};
...
impl JwtUtils {
    pub fn new(config: &Config) -> Result<Self, String> {
        ...
        Ok(Self {
            config: JwtConfig {
                ...
                algorithm,
                issuer: "foims-server".to_string(),
                audience: "foims-client".to_string(),
                leeway: 30,
            },
    ...
    fn get_jwt_secret(config_secret: &str) -> Result<String, String> {
        // 优先从环境变量读取
        if let Ok(env_secret) = std::env::var("FOIMS_JWT_SECRET")
            && !env_secret.is_empty()
        {
        ...
        if config_secret.is_empty() {
            Err("JWT 密钥未配置：请设置环境变量 FOIMS_JWT_SECRET，或在配置文件 jwt.secret 中提供长度不少于 32 字符的密钥".to_string())
        } else {
...
pub fn get_client_info_from_parts(parts: &axum::http::request::Parts) -> (String, String) {
    let ip_address = foims_common::net::get_real_ip_from_parts(parts);
    let user_agent = foims_common::net::get_user_agent_from_parts(parts);
...
mod tests {
    ...
    fn make_claims(exp: usize, iat: usize) -> JwtClaims {
        JwtClaims {
            ...
            exp,
            iat,
            iss: "foims-server".to_string(),
            jti: Uuid::new_v4().to_string(),
            aud: "foims-client".to_string(),
            token_type: "access".to_string(),
    ...
    fn test_access_token_roundtrip_claims() {
        ...
        assert_eq!(claims.iss, "foims-server");
        assert_eq!(claims.aud, "foims-client");
    ...
    fn test_legacy_token_without_remember_me_claim_deserializes_to_none() {
        ...
        let now = Utc::now().timestamp() as usize;
        let payload = format!(
            r#"{{"sub":"{}","username":"bob","role":"admin","exp":{},"iat":{},"iss":"foims-server","jti":"{}","aud":"foims-client","token_type":"refresh","device_fingerprint":null,"ip_address":null}}"#,
            Uuid::new_v4(),
            now + 600,
            now,
            Uuid::new_v4()
...
pub async fn cleanup_expired_revoked_tokens(pool: &sqlx::PgPool) -> Result<u64, sqlx::Error> {
    ...
    if deleted_count > 0 {
        ...
        foims_common::log_debug!("log.token.revoked_cleaned", count = deleted_count);
    }
