...
pub async fn get_ip_details<P: DbProvider>(
    ...
    let filters = IpListFilters {
        search: (!search.is_empty()).then(|| foims_common::net::escape_like(search)),
        status: (!status.is_empty()).then(|| status.to_string()),
        device_name: (!device_name.is_empty()).then(|| foims_common::net::escape_like(device_name)),
...
pub async fn create_device_ip<P: DbProvider>(
    ...
    Ok(foims_common::ok_json(mapping, "server.ip.created"))
}
...
pub async fn auto_assign_ip<P: DbProvider>(
    State(state): State<Arc<P>>,
    meta: RequestMeta,
    AppJson(req): AppJson<foims_models::AutoAssignIpRequest>,
) -> Result<Response, AppError> {
    req.validate()?;
    ...
    Ok(foims_common::ok_json(mapping, "server.ip.auto_assigned"))
}
