...
fn get_table_columns() -> std::collections::HashMap<&'static str, Vec<&'static str>> {
    let mut columns: std::collections::HashMap<&'static str, Vec<&'static str>> = std::collections::HashMap::new();
    ...
    columns.insert("workstation_networks", vec!["id", "workstation_id", "network_id", "created_at", "updated_at"]);
...
async fn create_database(config: &crate::config::DatabaseConfig) -> Result<(), String> {
    ...
    // 使用 postgres 数据库创建新数据库，设置连接限制避免占用
...
pub async fn import_database_from_file(
    ...
    while let Some(mut field) = payload.try_next().await.map_err(|e| actix_web::error::ErrorBadRequest(e))? {
...
async fn create_tables(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
    ...
    sqlx::query(
        r#"CREATE TABLE IF NOT EXISTS workstation_networks (
            id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
async fn create_views(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
    ...
    sqlx::query(r#"
        ...
        SELECT 
            ...
            CAST(imm.ip_address AS TEXT) as ip_address,
            imm.ip_version,
            imm.mac_address,
            imm.hostname,
            imm.status,
