...
pub async fn run(pool: &PgPool) -> Result<(), Error> {
    ...
    migrate_switch_view_add_network_columns(pool).await?;
    migrate_switch_view_use_network_id(pool).await?;
    Ok(())
}
...
async fn migrate_switch_view_add_network_columns(pool: &PgPool) -> Result<(), Error> {
    let result = sqlx::query(
        "SELECT COUNT(*) as count FROM schema_migrations WHERE version = 'switch_view_add_network_columns'",
    )
    .fetch_one(pool)
    .await?;
    ...
    if count == 0 {
        ...
        .await
        {
            warn!("创建switches_with_details视图失败: {}", e);
        }

        sqlx::query(
            r"INSERT INTO schema_migrations (version, description) VALUES ('switch_view_add_network_columns', '添加position_network_id和network_region_id列到switches_with_details视图')"
        )
        .execute(pool)
        .await?;
    }
...
async fn migrate_switch_view_use_network_id(pool: &PgPool) -> Result<(), Error> {
    let result = sqlx::query(
        "SELECT COUNT(*) as count FROM schema_migrations WHERE version = 'switch_view_use_network_id'",
    )
    .fetch_one(pool)
    .await?;
    ...
    if count == 0 {
        ...
        sqlx::query(
            r"INSERT INTO schema_migrations (version, description) VALUES ('switch_view_use_network_id', '更新switches_with_details视图使用ips.network_id替代position_network_id')"
        )
        .execute(pool)
        .await?;
    }
