...
async fn migrate_ips_add_room_network_id(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
    let result = sqlx::query(
...
async fn migrate_ip_with_details_room_network_id(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
    let result = sqlx::query(
