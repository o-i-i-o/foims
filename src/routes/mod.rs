            ...
            web::scope("/api")
                ...
                .service(
                    web::scope("/resources")
                        ...
                        // 物理链路管理
                        .service(
                            web::scope("/cable-links")
                                .route("", web::get().to(get_cable_links))
                                .route("", web::post().to(create_cable_link))
                                .route("/path", web::get().to(get_cable_path))
                                .route("/{id}", web::get().to(get_cable_link))
                                .route("/{id}", web::put().to(update_cable_link))
                                .route("/{id}", web::delete().to(delete_cable_link)),
                        )
                        // 设备模板管理
                        .service(
                        ...
                        .service(
                            web::scope("/devices")
                                ...
                                .route("/switch-ports", web::get().to(get_all_switch_ports))
                                .route("/interfaces", web::get().to(get_all_device_interfaces))
                                ...
                                .route(
                                    "/switch-ports/{port_id}",
                                    web::delete().to(delete_switch_port),
                                )
                                .route(
                                    "/interfaces/{interface_id}",
                                    web::get().to(get_device_interface),
                                )
                                .route(
                                    "/interfaces/{interface_id}",
                                    web::put().to(update_device_interface),
                                )
                                .route(
                                    "/interfaces/{interface_id}",
                                    web::delete().to(delete_device_interface),
                                ),
                        ),
                )
                // 日志管理路由
