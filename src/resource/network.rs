...
pub async fn get_networks(
    ...
    let networks = if let Some(region_id) = region_id {
        // 根据region_id过滤查询
        match sqlx::query(
    ...
    Ok(
        HttpResponse::Ok().json(ApiResponse::<Vec<Network>>::success(
            networks,
            "网络获取成功",
        )),
    )
...
pub async fn get_network(
    ...
    let network = match sqlx::query(
        r#"SELECT n.id, n.name, n.network_region_id, nt.name as network_region, 
                  ...
                  n.ipv4_dns::TEXT, n.ipv6_dns::TEXT, 
...
pub async fn update_network(
    ...
    if let Err(err) = sqlx::query(
        "UPDATE network_cidrs SET 
         ...
         ipv4_dns = COALESCE(CAST($7 AS INET), ipv4_dns),
         ipv6_dns = COALESCE(CAST($8 AS INET), ipv6_dns),
...
// 网络区域相关路由
// 获取所有网络区域
pub async fn get_network_regions(pool: web::Data<DbPool>) -> Result<HttpResponse> {
    ...
    Ok(
        HttpResponse::Ok().json(ApiResponse::<Vec<NetworkRegion>>::success(
            network_regions,
            "网络区域获取成功",
        )),
    )
