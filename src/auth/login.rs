        ...
        None => {
            ...
            return Ok(req.into_response(
                HttpResponse::InternalServerError()
                    .json(ApiResponse::<()>::error_i18n("api.server_error", &user_lang))
                    .map_into_right_body(),
            ));
        }
    };
...
pub async fn refresh_token(
    ...
    let new_refresh_token = jwt_utils.generate_refresh_token(&user_id, &claims.username, &claims.role, Some(&current_fingerprint), Some(&ip_address), remember_me).unwrap();
...
pub async fn disable_two_factor(
    ...
    if let Some(encrypted_secret) = secret {
        ...
        if let Ok(secret_bytes) = Secret::Encoded(secret).to_bytes()
            && let Ok(totp) = TOTP::new(Algorithm::SHA1, 6, 1, 30, secret_bytes, Some("IPMA".to_string()), target_username)
