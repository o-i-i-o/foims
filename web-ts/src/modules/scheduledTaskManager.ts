...
export async function initScheduledTaskManager(): Promise<void> {
...
async function loadScheduledTasks(): Promise<void> {
  try {
    ...
    const tasks = (result.data as { items?: ScheduledTask[] })?.items || [];
...
async function editScheduledTask(id: string): Promise<void> {
...
export async function createScheduledTask(taskData: Record<string, unknown>): Promise<boolean> {
  try {
