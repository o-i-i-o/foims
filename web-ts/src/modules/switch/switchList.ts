...
let switchTableClickHandler: ((e: Event) => void) | null = null;

export async function loadSwitchesData(searchTerm = ""): Promise<void> {
  ...
  try {
    ...
    const result = await apiGet<Switch[] | PagedData<Switch>>(url);
    const data = result.success ? result.data : { items: [], total: 0 };
    const switches = (data as PagedData<Switch>)?.items || (Array.isArray(data) ? data : []);
    ...
    renderTable("#switches-table", {
      ...
      columns: [
        ...
        { field: "device_type", render: (v: unknown) => (v as string) || "-" },
        { field: "ip_address", render: (v: unknown) => v as string },
        { field: "mac_address", render: (v: unknown) => (v as string) || "-" },
        { field: "model", render: (v: unknown) => (v as string) || "-" },
        { field: "vendor", render: (v: unknown) => (v as string) || "-" },
        { field: "location", render: (v: unknown) => (v as string) || "-" },
        {
          field: "snmp_version",
          render: (v: unknown, row: unknown) => {
            const sw = row as Switch;
            return `<span class="status-badge ${sw.snmp_community || sw.snmp_username ? "status-active" : "status-inactive"}">${v || "-"}</span>`;
          },
        },
        { field: "parent_switch_name", render: (v: unknown) => (v as string) || "-" },
        {
          field: "id",
          render: (v: unknown, row: unknown) => {
            const sw = row as Switch;
    ...
    if ((data as PagedData<Switch>).total !== undefined) {
      appendPaginationToTable("#switches-table", data as PagedData<Switch>, (p: number) => {
...
function bindSwitchButtonsEvents(): void {
  const table = elementCache.get("switches-table") as HTMLElement | null;
  if (!table) return;
...
export async function fetchSwitchById(id: string): Promise<Switch | null> {
  try {
    const result = await apiGet<Switch>(`/api/switches/${id}`);
    if (result.success) {
...
export async function submitSwitchForm(formData: SwitchFormData): Promise<boolean> {
