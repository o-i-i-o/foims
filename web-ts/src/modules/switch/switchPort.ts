...
interface PortTypePattern {
  typeName: string;
  regex: RegExp;
  subGroup: boolean;
}

const PORT_TYPE_PATTERNS: PortTypePattern[] = [
...
export async function loadSwitchPortsData(page = 1, searchTerm = ""): Promise<void> {
  ...
  try {
    ...
    const result = await apiGet<SwitchPort[] | PagedData<SwitchPort>>(url);
    const data = result.success ? result.data : { items: [], total: 0 };
    const ports = (data as PagedData<SwitchPort>)?.items || (Array.isArray(data) ? data : []);

    renderTable("#switch-ports-table", {
      data: ports,
      columns: [
        { field: "switch_name", render: (v: unknown, row: Record<string, unknown>) => v ? `${v} (${row.switch_ip || ""})` : currentSwitchName },
        { field: "port_number", render: (v: unknown) => String(v) },
        { field: "port_name", render: (v: unknown) => (v as string) || "-" },
        { field: "port_type", render: (v: unknown) => String(v) },
        { field: "vlan_id", render: (v: unknown) => v != null ? String(v) : "-" },
        { field: "status", render: (v: unknown) => `<span class="status-badge ${v === "up" ? "status-active" : "status-inactive"}">${v}</span>` },
        { field: "speed", render: (v: unknown) => v != null ? String(v) : "-" },
        {
          field: "id",
          render: (v: unknown) => `
    ...
    if ((data as PagedData<SwitchPort>).total !== undefined) {
      appendPaginationToTable("#switch-ports-table", data as PagedData<SwitchPort>, (p: number) => loadSwitchPortsData(p, searchTerm));
    }
  } catch (error) {
...
export async function loadSwitchPortsBySwitchId(switchId: string): Promise<SwitchPort[]> {
  try {
    const result = await apiGet<SwitchPort[] | PagedData<SwitchPort>>(`/api/switches/${switchId}/ports?page_size=1000`);

    const searchContainer = document.querySelector("#switch-ports-list-tab .search-container");

    if (!searchContainer) {
      renderTable("#switch-ports-table", {
        data: result.success ? (Array.isArray(result.data) ? result.data : (result.data as PagedData<SwitchPort>)?.items || []) : [],
        columns: [
          { field: "port_number", render: (v: unknown) => String(v) },
          { field: "port_name", render: (v: unknown) => (v as string) || "-" },
          { field: "port_type", render: (v: unknown) => String(v) },
    ...
    let addBtn = elementCache.get("add-switch-port-btn") as HTMLButtonElement | null;
    if (!addBtn) {
    ...
    let snmpPortsBtn = elementCache.get("sync-snmp-ports-btn") as HTMLButtonElement | null;
    if (!snmpPortsBtn) {
      ...
      snmpPortsBtn.addEventListener("click", () => {
        ...
        if (sid) syncPortsFromSnmp(sid);
      });
      actionButtonsContainer.appendChild(snmpPortsBtn);
    }

    const ports = result.success
      ? (Array.isArray(result.data) ? result.data : (result.data as PagedData<SwitchPort>)?.items || [])
      : [];
...
export async function manageSwitchPorts(switchId: string, switchName?: string): Promise<void> {
  setCurrentSwitchId(switchId);
  setCurrentSwitchName(switchName || "");

  try {
    const switchResult = await apiGet<Record<string, unknown>>(`/api/switches/${switchId}`);
    if (!switchResult.success) {
      showToast(t("switch.fetch_info_failed") || "获取交换机信息失败，请检查网络连接", "error");
      return;
    }

    const switchData = switchResult.data as Record<string, unknown>;
    ...
    const portsResult = await apiGet<SwitchPort[] | PagedData<SwitchPort>>(`/api/switches/${switchId}/ports?page_size=1000`);
    ...
    if (portsResult.data) {
      ...
      } else if ((portsResult.data as PagedData<SwitchPort>).items && Array.isArray((portsResult.data as PagedData<SwitchPort>).items)) {
        ports = (portsResult.data as PagedData<SwitchPort>).items;
      }
    }
...
export function groupPorts(ports: SwitchPort[]): Record<string, SwitchPort[]> {
  ...
  const rawGroups: Record<string, SwitchPort[]> = {};
  ...
  const finalGroups: Record<string, SwitchPort[]> = {};
  const smallGroupPorts: SwitchPort[] = [];
...
export function showPortGroupsModal(switchName: string, portGroups: Record<string, SwitchPort[]>, switchId: string): void {
  openModal("switch-ports-group-modal");

  const modal = elementCache.get("switch-ports-group-modal");
  const title = elementCache.get("switch-ports-group-modal-title") as HTMLElement | null;
  const container = document.querySelector(".port-groups-container");
  const addPortBtn = elementCache.get("add-port-btn") as HTMLButtonElement | null;
  ...
  (container as HTMLElement).onclick = (e: MouseEvent) => {
    const target = e.target as HTMLElement;
    const portItem = target.closest(".port-item") as HTMLElement | null;
    if (portItem) {
      openPortDetailModal({
  ...
  const getSnmpPortsBtn = elementCache.get("get-snmp-ports-btn") as HTMLButtonElement | null;
  if (getSnmpPortsBtn) {
    getSnmpPortsBtn.onclick = async () => {
      ...
      if (success) {
        ...
        if (portsResult.success && portsResult.data) {
          ...
          if (Array.isArray(portsResult.data)) {
            ports = portsResult.data;
          } else if ((portsResult.data as PagedData<SwitchPort>).items) {
            ports = (portsResult.data as PagedData<SwitchPort>).items;
          }
...
function openPortDetailModal(portData: PortDetailData): void {
  openModal("switch-port-detail-modal");

  const modal = elementCache.get("switch-port-detail-modal");
  const title = elementCache.get("switch-port-detail-modal-title") as HTMLElement | null;
  const form = elementCache.get("switch-port-form-expanded") as HTMLFormElement | null;
  const saveBtn = elementCache.get("save-port-btn") as HTMLButtonElement | null;
  const deleteBtn = elementCache.get("delete-port-btn") as HTMLButtonElement | null;
...
export function openSwitchPortModal(portData: SwitchPort | null = null, switchId: string | null = null): void {
  openModal("switch-port-modal");

  const modal = elementCache.get("switch-port-modal");
  const title = elementCache.get("switch-port-modal-title") as HTMLElement | null;
  const form = elementCache.get("switch-port-form") as HTMLFormElement | null;
  ...
  if (portData) {
    if (title) title.textContent = t("switch.edit_port") || "编辑端口";
    ...
    if (title) title.textContent = t("switch.add_port") || "添加端口";
...
export async function submitSwitchPortForm(): Promise<boolean> {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (currentSwitchId) {
        try {
          ...
          if (currentSwitchName) {
            const portsResult = await apiGet<SwitchPort[] | PagedData<SwitchPort>>(`/api/switches/${currentSwitchId}/ports?page_size=1000`);
            if (portsResult.success && portsResult.data) {
              let ports: SwitchPort[] = [];
              if (Array.isArray(portsResult.data)) {
                ports = portsResult.data;
      ...
      const errorMsg = (result.message ?? (t("common.operation_failed") || "操作失败，请检查输入信息")) as string;
...
export async function editSwitchPort(id: string): Promise<void> {
  try {
    const result = await apiGet<SwitchPort>(`/api/switches/ports/${id}`);
    if (result.success) {
      openSwitchPortModal(result.data as SwitchPort);
    } else {
...
export async function deleteSwitchPort(id: string | number): Promise<boolean> {
  const currentSwitchId = getCurrentSwitchId();
  ...
  if (!confirm(t("switch.confirm_delete_port") || "确定要删除此端口吗？")) return false;
  try {
...
export function extractPortNumber(portNumber: string): number {
...
export function extractPortLastNumber(portNumber: string): string {
