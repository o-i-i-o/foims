import type { ApiResponse, ApiResult, BlobResponse } from "../types/api.js";
...
export class ApiClient {
  static #pendingRequests = new Map<string, Promise<ApiResult>>();
  static #requestTimestamps = new Map<string, number>();

  static THROTTLE_INTERVAL = 1000;

  static requestQueue: QueueItem[] = [];
  ...
  static async request(url: string, options: RequestInit & { skipAuthCheck?: boolean } = {}, retryCount = 0): Promise<ApiResult> {
    ...
    const { skipAuthCheck, ...restOptions } = options as RequestInit & { skipAuthCheck?: boolean };
  ...
  private static async processRequest(url: string, options: RequestInit, retryCount: number, requestKey: string): Promise<ApiResult> {
  ...
  private static processQueue(): void {
    while (this.currentRequests < this.MAX_CONCURRENT_REQUESTS && this.requestQueue.length > 0) {
      ...
      if (queueItem) {
        ...
        ).then(queueItem.resolve).catch((error: Error) => {
  ...
  private static async makeRequest(url: string, options: RequestInit, retryCount: number, requestKey: string): Promise<ApiResult> {
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          if (this.isPublicAuthEndpoint(url)) {
            try {
              const errorData = await response.json() as { message?: string; error_type?: string; error_details?: Record<string, unknown>; suggested_action?: string };
              return {
                success: false,
  ...
  static isPublicAuthEndpoint(url: string): boolean {
  ...
  static async get<T = unknown>(url: string): Promise<ApiResponse<T>> {
    return this.request(url) as Promise<ApiResponse<T>>;
  }

  static async post<T = unknown>(url: string, data?: unknown, options: RequestInit & { skipAuthCheck?: boolean } = {}): Promise<ApiResponse<T>> {
    const body = data instanceof FormData ? data : JSON.stringify(data);
    return this.request(url, { method: "POST", body, ...options }) as Promise<ApiResponse<T>>;
  }

  static async put<T = unknown>(url: string, data: unknown, options: RequestInit & { skipAuthCheck?: boolean } = {}): Promise<ApiResponse<T>> {
    return this.request(url, { method: "PUT", body: JSON.stringify(data), ...options }) as Promise<ApiResponse<T>>;
  }

  static async delete<T = unknown>(url: string, data: unknown = null, options: RequestInit & { skipAuthCheck?: boolean } = {}): Promise<ApiResponse<T>> {
    const deleteOptions: RequestInit & { skipAuthCheck?: boolean } = { method: "DELETE", ...options };
    if (data) {
      deleteOptions.body = JSON.stringify(data);
    }
    return this.request(url, deleteOptions) as Promise<ApiResponse<T>>;
  }
