...
import type { IpAssignment, IpValidationResult, CidrInfo, ResourceType, IpConfig, Switch, SwitchPort, Network, NetworkRegion } from "../types/resources.js";
import { showToast } from "./toast.js";

const NETWORK_REGION_CHANGED_EVENT = "ipma:network-region-changed";

const regionChangeCallbacks = new WeakMap<(regionId: string, regionName: string) => void, (e: Event) => void>();
const networkChangeCallbacks = new Set<(networkId: string, networkName: string, networkRegionId: string) => void>();
...
function ipv6ToInt(ipv6: string): bigint | null {
  let ip = ipv6.split("%")[0];
...
function ipv4CidrToRange(cidr: string): IpRange | null {
...
function ipv6CidrToRange(cidr: string): IpRange | null {
  const [ip, prefixLen] = cidr.split("/");
  const prefix = parseInt(prefixLen, 10);
...
function isIpInCidr(ipAddress: string, cidr: string | null | undefined): boolean {
  if (!cidr) return true;
...
export function dispatchNetworkRegionChange(regionId: string, regionName = ""): void {
  document.dispatchEvent(new CustomEvent(NETWORK_REGION_CHANGED_EVENT, {
    detail: { regionId, regionName },
  }));
}

export function onNetworkRegionChange(callback: (regionId: string, regionName: string) => void): void {
  if (typeof callback !== "function") {
    console.error("onNetworkRegionChange: callback must be a function");
    return;
  }
  const wrapper = (e: Event) => {
    const detail = (e as CustomEvent).detail as { regionId: string; regionName: string };
    callback(detail.regionId, detail.regionName);
  };
  regionChangeCallbacks.set(callback, wrapper);
  document.addEventListener(NETWORK_REGION_CHANGED_EVENT, wrapper);
}

export function offNetworkRegionChange(callback: (regionId: string, regionName: string) => void): void {
...
export function onNetworkChange(callback: (networkId: string, networkName: string, networkRegionId: string) => void): void {
  if (typeof callback !== "function") {
...
export function offNetworkChange(callback: (networkId: string, networkName: string, networkRegionId: string) => void): void {
  networkChangeCallbacks.delete(callback);
}

function dispatchNetworkChange(networkId: string, networkName: string, networkRegionId: string): void {
...
const CONFIG: Record<string, IpConfig> = {
  workstation: {
...
let switchesCache: Switch[] | null = null;
...
export class IpConfigManager {
  ...
  setExcludeSwitchId(switchId: number | string): void {
  ...
  getNetworkCidr(networkId: string): string | null {
    const network = this.networks.find(n => String(n.id) === networkId);
    return network?.ipv4_cidr || network?.ipv6_cidr || null;
  }

  getCidrForIp(networkId: string, ipAddress: string): CidrInfo {
    ...
    return { cidr: cidr || null, hasCidr, isV6 };
  }

  async loadIps(ips: IpAssignment[]): Promise<void> {
    this.clear();
    if (!ips || ips.length === 0) {
      await this.addIpRow();
      return;
    ...
    try {
      let regions: NetworkRegion[] = [];
      let allNetworks: Network[] = [];

      if (this.config.loadNetworksByRegion) {
        const regionsResult = await apiGet<NetworkRegion[] | PagedData<NetworkRegion>>("/api/resources/network-regions?page_size=1000");
        if (!regionsResult.success) {
          throw new Error("加载网络区域失败");
        }
        regions = extractItems(regionsResult) as NetworkRegion[];

        const regionIds = new Set<string>();
        for (const ip of ips) {
          if (ip.network_region_id) {
            regionIds.add(String(ip.network_region_id));
          }
        }
        ...
        if (regionIds.size === 0 && ips.some(ip => ip.network_id)) {
          ...
          const networkPromises = uniqueNetworkIds.map(id =>
            apiGet<Network>(`/api/resources/networks/${id}`).catch(() => ({ success: false }) as ApiResponse),
          );
          const networkResults = await Promise.all(networkPromises);
          for (const result of networkResults) {
            if (result?.success && result.data) {
              const network = result.data as Network;
              if (network.network_region_id) {
                regionIds.add(String(network.network_region_id));
              }
        ...
        if (regionIds.size > 0) {
          const networkPromises = Array.from(regionIds).map(regionId => {
            ...
            return url ? apiGet<Network[] | PagedData<Network>>(url).catch(() => null) : null;
          }).filter(Boolean);

          const networkResults = await Promise.all(networkPromises as Promise<ApiResponse<Network[] | PagedData<Network>> | null>[]);
          for (const result of networkResults) {
            if (result?.success && result.data) {
              const regionNetworks = extractItems(result);
              allNetworks = allNetworks.concat(regionNetworks);
            }
          }
        }
      } else if (this.config.idSelector) {
        const id = (document.getElementById(this.config.idSelector) as HTMLSelectElement | null)?.value;
        if (!id) {
        ...
        if (url) {
          ...
          allNetworks = extractItems(result);
        }

        const regionMap = new Map<string, NetworkRegion>();
        for (const network of allNetworks) {
          if (network.network_region_id && network.network_region) {
            regionMap.set(String(network.network_region_id), {
              id: network.network_region_id as number,
      ...
      showToast((error as Error).message || "加载IP数据失败", "error");
      await this.addIpRow();
    }
  }
  ...
  getIps(): IpAssignment[] {
    ...
    const ips: IpAssignment[] = [];

    rows.forEach(row => {
      const networkRegionSelect = row.querySelector(`.${this.config.classPrefix}-ip-network-region-select`) as HTMLSelectElement | null;
      const networkSelect = row.querySelector(`.${this.config.classPrefix}-ip-network-select`) as HTMLSelectElement | null;
      const ipAddressInput = row.querySelector(`.${this.config.classPrefix}-ip-address-input`) as HTMLInputElement | null;
      const macAddressInput = row.querySelector(`.${this.config.classPrefix}-ip-mac-address-input`) as HTMLInputElement | null;
      const switchSelect = row.querySelector(`.${this.config.classPrefix}-switch-select`) as HTMLSelectElement | null;
      const switchPortSelect = row.querySelector(`.${this.config.classPrefix}-switch-port-select`) as HTMLSelectElement | null;
  ...
  validateIps(): IpValidationResult {
    const container = this.getContainer();
    if (!container) return { valid: true, errors: [], ips: [] };
  ...
  async addIpRow(initialData: IpAssignment | null = null): Promise<void> {
    const container = this.getContainer();
    if (!container) return;
  ...
  async loadNetworks(initialData: IpAssignment | null): Promise<Network[]> {
    let url: string | null;
    if (this.config.idSelector) {
      ...
      if (!regionId && initialData?.network_id) {
        const allNetworksResult = await apiGet<Network[] | PagedData<Network>>("/api/resources/networks?page_size=10000");
        if (allNetworksResult.success && allNetworksResult.data) {
          const allNetworks = extractItems(allNetworksResult);
          const network = allNetworks.find(n => String(n.id) === initialData.network_id);
          if (network) {
            regionId = String(network.network_region_id);
          }
        }
      }
      if (regionId) {
  ...
  private createIpRowElement(regions: NetworkRegion[], networks: Network[]): HTMLDivElement {
  ...
  private async bindRowEvents(row: HTMLDivElement, networks: Network[], initialData: IpAssignment | null): Promise<void> {
    ...
    const regionSelect = row.querySelector(`.${this.config.classPrefix}-ip-network-region-select`) as HTMLSelectElement | null;
    const networkSelect = row.querySelector(`.${this.config.classPrefix}-ip-network-select`) as HTMLSelectElement | null;
    const ipInput = row.querySelector(`.${this.config.classPrefix}-ip-address-input`) as HTMLInputElement | null;
    const macInput = row.querySelector(`.${this.config.classPrefix}-ip-mac-address-input`) as HTMLInputElement | null;
    const switchSelect = row.querySelector(`.${this.config.classPrefix}-switch-select`) as HTMLSelectElement | null;
    const portSelect = row.querySelector(`.${this.config.classPrefix}-switch-port-select`) as HTMLSelectElement | null;

    let currentNetworks = networks;
      ...
      regionSelect.addEventListener("change", async () => {
        ...
        if (this.config.loadNetworksByRegion && regionId) {
          ...
          if (url) {
            ...
            if (result.success && result.data) {
              const regionNetworks = extractItems(result);
              currentNetworks = regionNetworks;
              this.networks = regionNetworks;
            }
          }
        }

        const filtered = currentNetworks.filter(n => String(n.network_region_id) === regionId);
        ...
        if (filtered.length === 1) {
          networkSelect.value = String(filtered[0].id);
        }
    ...
    if (networkSelect) {
      networkSelect.addEventListener("change", () => {
        ...
        const network = this.networks.find(n => String(n.id) === networkId);
        if (network) {
          this.filterSwitchesByRegion(switchSelect, portSelect, String(network.network_region_id));
          if (this.resourceType === "switch") {
            dispatchNetworkChange(networkId, network.name, String(network.network_region_id));
          }
        }
      });
    }
    ...
    if (initialData) {
      let regionId = initialData.network_region_id;
      if (!regionId && initialData.network_id) {
        const network = this.networks.find(n => String(n.id) === initialData.network_id);
        if (network) {
          regionId = String(network.network_region_id);
        }
      }
      ...
      if (switchId && switchSelect) {
        const normalizedSwitchId = typeof switchId === "string" ? switchId.toLowerCase() : String(switchId);
      ...
      if (portId && portSelect) {
        ...
        const normalizedPortId = typeof portId === "string" ? portId.toLowerCase() : String(portId);
  ...
  private async loadSwitches(switchSelect: HTMLSelectElement | null, portSelect: HTMLSelectElement | null): Promise<void> {
    if (!switchSelect || !portSelect) return;

    try {
      const result = await apiGet<Switch[] | PagedData<Switch>>("/api/switches");
      const switchesData = extractItems(result);
  ...
  private filterSwitchesByRegion(switchSelect: HTMLSelectElement | null, portSelect: HTMLSelectElement | null, regionId: string): void {
    ...
    if (regionId) {
      filteredSwitches = switchesCache.filter(sw => String(sw.network_region_id) === regionId);
    }
    ...
    if (currentSwitchId && filteredSwitches.some(sw => String(sw.id) === currentSwitchId)) {
      switchSelect.value = currentSwitchId;
    }
  }

  private async handleSwitchChange(switchSelect: HTMLSelectElement | null, portSelect: HTMLSelectElement | null): Promise<void> {
    if (!switchSelect || !portSelect) return;
    const switchId = switchSelect.value;
    portSelect.innerHTML = '<option value="">选择端口</option>';
    ...
    try {
      const result = await apiGet<SwitchPort[] | PagedData<SwitchPort>>(`/api/switches/${switchId}/ports`);
      if (result.success && result.data) {
        const ports = extractItems(result);

        ports.sort((a, b) => {
          const getNum = (s: string | undefined): number => {
...
const managerInstances: Partial<Record<ResourceType, IpConfigManager>> = {};

function getManager(type: ResourceType): IpConfigManager {
  if (!managerInstances[type]) {
    managerInstances[type] = new IpConfigManager(type);
  }
  return managerInstances[type]!;
}

export async function addIpAddressField(resourceType: ResourceType): Promise<void> {
  const manager = getManager(resourceType);
  await manager.addIpRow();
}

export function bindButton(buttonId: string, resourceType: ResourceType): void {
...
export const handleWorkstationRoomChange = async (): Promise<void> => {
  const manager = getManager("workstation");
  manager.clear();
  await manager.addIpRow();
};

export const handleCabinetPositionCabinetChange = async (): Promise<void> => {
