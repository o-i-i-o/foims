        ...
        refreshBtn.addEventListener("click", () => {
            ...
            if (activeTab) {
                ...
                if (tabId === "notifications")
                    ...
                    const sv = searchInput ? searchInput.value : "";
                    loadLogsData(tabId, { action: sv, page: 1 });
                }
            }
        });
    }
...
export async function loadLogsData(logType = "operation", searchParams = {}) {
    try {
        ...
        if (data.success) {
            ...
            if (Array.isArray(data.data)) {
                ...
                if (Array.isArray(d.items)) {
                    logs = d.items;
                    pagination = { total: d.total, page: d.page, total_pages: d.total_pages, page_size: d.page_size };
                }
            ...
            if (logs.length > 0) {
                ...
                logs.forEach((log, index) => {
                    ...
                    if (logType === "operation") {
                        ...
                        rowHtml += `<td>${new Date(log.created_at).toLocaleString()}</td><td>${escapeHtml(log.username) || "-"}</td><td>${escapeHtml(opText)}</td><td>${escapeHtml(resText) || "-"}</td><td>${resultText}</td><td>${escapeHtml(log.ip_address) || "-"}</td><td><button class="btn btn-sm btn-info view-log-details" data-log="${logData}">${t('logs.details')}</button></td>`;
                    }
                    else if (logType === "login") {
                        const loginResult = log.success ? t('common.success') : t('common.failed');
                        rowHtml += `<td>${new Date(log.created_at).toLocaleString()}</td><td>${escapeHtml(log.username)}</td><td>${escapeHtml(log.ip_address)}</td><td>${escapeHtml(log.user_agent) || "-"}</td><td>${loginResult}</td><td>${escapeHtml(log.error_message) || "-"}</td>`;
                    }
                    row.innerHTML = rowHtml;
                    tbody.appendChild(row);
                });
                if (pagination && pagination.total_pages > 1) {
                    appendPaginationToTable(`#${logType}-logs-table`, { total: pagination.total, page: pagination.page, page_size: pagination.page_size }, (p) => {
        ...
        if (tbody) {
            const cs = logType === "operation" ? 8 : 7;
            tbody.innerHTML = `<tr class="empty-row"><td colspan="${cs}" class="text-center">${t('common.load_failed_retry')}</td></tr>`;
        }
    }
}
export async function loadNotificationsData(filterStatus = 'all', page = 1) {
    try {
        ...
        const notifications = (Array.isArray(dataObj) ? dataObj : dataObj.items || dataObj);
        if (notifications.length > 0) {
            const startIndex = (page - 1) * 20;
            notifications.forEach((n, index) => {
                const row = document.createElement("tr");
                row.innerHTML = `<td class="index-column">${startIndex + index + 1}</td><td>${new Date(n.created_at).toLocaleString()}</td><td>${n.title}</td><td>${n.content}</td><td><span class="status-badge ${n.read ? "status-active" : "status-inactive"}">${n.read ? "已读" : "未读"}</span></td><td>${!n.read ? `<button class="btn btn-sm btn-primary mark-read" data-id="${n.id}">标记已读</button>` : ""}</td>`;
                tbody.appendChild(row);
            });
            const dr = dataObj;
            if (dr.total !== undefined)
                appendPaginationToTable("#notifications-table", dr, (p) => loadNotificationsData(filterStatus, p));
        }
        else {
...
function initLogEvents() {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (target.classList.contains("view-log-details")) {
            ...
            if (logData) {
                try {
                    showLogDetails(JSON.parse(decodeURIComponent(logData)));
                }
                catch (_err) {
                    try {
                        showLogDetails(JSON.parse(logData));
                    }
                    catch (_e2) {
...
function showLogDetails(log) {
    ...
    if (log.details) {
        try {
            ...
            detailsHtml = `<pre class="log-details-json">${JSON.stringify(d, null, 2)}</pre>`;
        }
        catch (_e) {
