...
function getResourceCallbacks(): Record<string, () => Promise<void>> {
  const createCallback = (module: string, method: string): (() => Promise<void>) => {
    return () => import(`./modules/${module}.js`).then((m: Record<string, (...args: unknown[]) => unknown>) => {
      const fn = m[method];
