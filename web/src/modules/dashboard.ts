...
interface DashboardStats {
  networks?: { networks?: number; regions?: number };
  ips?: { total?: number; active?: number; by_device_type?: Record<string, number>; by_status?: Record<string, number> };
  activity?: { operations_24h?: number; logins_24h?: number };
  users?: { total?: number; active?: number };
  locations?: { rooms?: number; cabinets?: number; workstations?: number; positions?: number };
  switches?: number;
...
export async function loadDashboardData(forceRefresh = false): Promise<void> {
  if (!forceRefresh) {
    const cached = cache.get<DashboardStats>(CACHE_KEY_STATS);
    if (cached) {
  ...
  const result = await safeAsync<ApiResponse<DashboardStats>>(
    () => apiGet<DashboardStats>("/api/system/dashboard-stats"),
    "加载仪表盘数据",
    { showToast: false },
  );
...
function updateDashboardUI(data: DashboardStats): void {
  const elements: Record<string, string | number> = {
...
async function loadTopLists(): Promise<void> {
  const cached = cache.get<TopListData>(CACHE_KEY_TOP_LISTS);
  if (cached) {
  ...
  const results = await Promise.allSettled([
    fetchTopData("/api/resources/networks?page_size=5"),
    fetchTopData("/api/resources/ip?page_size=5"),
    fetchTopData("/api/resources/rooms?page_size=5"),
    fetchTopData("/api/switches?page_size=5"),
    fetchTopData("/api/resources/cabinets?page_size=5"),
    fetchTopData("/api/logs/operation?page_size=5"),
  ]);

  const topLists: TopListData = {
...
async function fetchTopData(url: string): Promise<Record<string, unknown>[]> {
  const result = await apiGet(url);
  if (result.success && result.data) {
    const d = result.data as Record<string, unknown>;
    if (Array.isArray(d)) return d as Record<string, unknown>[];
    if (d.items && Array.isArray(d.items)) return d.items as Record<string, unknown>[];
    if (d.data && Array.isArray(d.data)) return d.data as Record<string, unknown>[];
  }
  return [];
}

function renderTopLists(data: TopListData): void {
...
function renderTopNetworks(items: Record<string, unknown>[]): void {
...
function renderTopIPs(items: Record<string, unknown>[]): void {
  ...
  container.innerHTML = items.slice(0, 5).map(ip => `
    <li>
      ...
      <span class="item-status status-${ip.status || "inactive"}">${getStatusText(ip.status as string)}</span>
    </li>
  `).join("");
}

function renderTopRooms(items: Record<string, unknown>[]): void {
  ...
  container.innerHTML = items.slice(0, 5).map(room => `
    <li>
      <div class="item-name">
        ...
        <div>
          <div>${room.name || "-"}</div>
          <div class="item-meta">${getRoomTypeName(room.room_type as string)}</div>
        </div>
      </div>
...
function renderTopSwitches(items: Record<string, unknown>[]): void {
...
function renderTopCabinets(items: Record<string, unknown>[]): void {
...
function renderTopLogs(items: Record<string, unknown>[]): void {
  ...
  container.innerHTML = items.slice(0, 5).map(log => `
    <li>
      <div class="item-name">
        <span class="item-icon">${getActionIcon(log.action as string)}</span>
        <div>
          <div>${log.action || "-"}</div>
          <div class="item-meta">${log.username || "-"} · ${formatTime(log.created_at as string)}</div>
        </div>
      </div>
...
function initDashboardClickHandlers(): void {
  ...
  dashboard.addEventListener("click", function (e: MouseEvent) {
    const target = e.target as HTMLElement;
    const card = target.closest(".stat-card.clickable, .dashboard-card") as HTMLElement | null;
    if (!card) return;
    ...
    if (nav) {
      ...
      if (tab) {
        setTimeout(() => {
          const tabBtn = document.querySelector(`.tab-btn[data-tab="${tab}"]`) as HTMLElement | null;
          if (tabBtn) {
            tabBtn.click();
          }
        }, 100);
...
function renderCharts(data: DashboardStats): void {
...
function renderDeviceTypeChart(deviceTypes: Record<string, number>): void {
...
function renderIpStatusChart(statusData: Record<string, number>): void {
...
function renderRoomTypeChart(roomTypes: Record<string, number>): void {
...
async function loadFallbackData(): Promise<DashboardStats | null> {
  try {
    ...
    const extractLength = (resp: ApiResponse): number => {
      if (!resp.success || !resp.data) return 0;
    ...
    const extractItems = (resp: ApiResponse): Record<string, unknown>[] => {
    ...
    const dashboardData: DashboardStats = {
      networks: {
        networks: extractLength(networksResponse),
        regions: extractLength(regionsResponse),
      },
      ips: {
        total: extractLength(ipResponse),
        active: extractItems(ipResponse).filter(({ status }) => status === "active").length,
      },
      activity: {
        operations_24h: 0,
      },
...
export function getStatsCache(): DashboardStats | null {
  return cache.get<DashboardStats>(CACHE_KEY_STATS);
}
