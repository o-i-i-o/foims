...
export async function viewArpTable(switchId: string): Promise<void> {
  ...
  let ipv4SearchHandler: (() => void) | null = null;
  let ipv6SearchHandler: (() => void) | null = null;
  let ipv4Entries: ArpEntry[] = [];
  let ipv6Entries: ArpEntry[] = [];

  const loadArpData = async () => {
    const loadingEl = modal.querySelector("#arp-loading") as HTMLElement | null;
    const contentEl = modal.querySelector("#arp-content") as HTMLElement | null;
    const nameEl = modal.querySelector("#arp-switch-name") as HTMLElement | null;
    const syncBtn = modal.querySelector("#sync-mac-btn") as HTMLButtonElement | null;
    ...
    try {
      ...
      if (!switchResult.success) {
        if (loadingEl) loadingEl.innerHTML = `<p style="color: red;">${t("switch.fetch_info_failed") || "获取交换机信息失败"}</p>`;
        return;
      }

      const switchData = switchResult.data as Record<string, unknown>;
      if (nameEl) nameEl.textContent = `- ${switchData.name}`;

      const result = await apiGet<ArpEntry[]>(`/api/switches/${switchId}/macs`);

      if (result.success) {
        const entries = result.data as ArpEntry[] || [];
        if (nameEl) nameEl.textContent = `- ${switchData.name} (${entries.length}${t("switch.entries") || "条"})`;
        ...
        if (ipv4TabBtn) ipv4TabBtn.textContent = `IPv4 (${ipv4Entries.length})`;
        if (ipv6TabBtn) ipv6TabBtn.textContent = `IPv6 (${ipv6Entries.length})`;

        const ipv4Container = modal.querySelector("#ipv4-table-container");
        const ipv6Container = modal.querySelector("#ipv6-table-container");
        if (ipv4Container) ipv4Container.innerHTML = renderMacTable(ipv4Entries, "ipv4");
        if (ipv6Container) ipv6Container.innerHTML = renderMacTable(ipv6Entries, "ipv6");

        if (ipv4Container) bindCollapseEvents(ipv4Container as HTMLElement);
        if (ipv6Container) bindCollapseEvents(ipv6Container as HTMLElement);
        ...
        if (isFirstLoad) {
          ...
          tabBtns.forEach(btn => {
            btn.addEventListener("click", () => {
              ...
              const targetTab = modal.querySelector(`#${(btn as HTMLElement).dataset.tab}-tab`);
              if (targetTab) targetTab.classList.add("active");
            });
          });

          const ipv4Search = modal.querySelector("#ipv4-search") as HTMLInputElement | null;
          const ipv6Search = modal.querySelector("#ipv6-search") as HTMLInputElement | null;
          ...
          ipv4SearchHandler = () => {
            ...
            if (container) {
              ...
              bindCollapseEvents(container as HTMLElement);
            }
          };
        ...
        if (loadingEl) loadingEl.innerHTML = `<p style="color: red;">${t("switch.load_mac_failed") || "加载MAC表失败"}: ${result.message || t("common.unknown_error") || "未知错误"}</p>`;
      }
    } catch (err) {
      const error = err as Error;
      if (loadingEl) loadingEl.innerHTML = `<p style="color: red;">${t("common.load_failed") || "加载失败"}: ${error.message}</p>`;
    } finally {
  ...
  const syncMacData = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (loadingEl) loadingEl.innerHTML = `<p style="color: red;">${t("switch.sync_failed") || "同步失败"}: ${result.message || t("common.unknown_error") || "未知错误"}</p>`;
      }
    } catch (err) {
      const error = err as Error;
      if (loadingEl) loadingEl.innerHTML = `<p style="color: red;">${t("switch.sync_failed") || "同步失败"}: ${error.message}</p>`;
    } finally {
      if (syncBtn) syncBtn.disabled = false;
    }
  };
...
export function renderMacTable(entries: ArpEntry[], type: "ipv4" | "ipv6"): string {
  if (entries.length === 0) {
...
export function bindCollapseEvents(container: HTMLElement): void {
  container.querySelectorAll(".network-group-header").forEach(header => {
    header.addEventListener("click", () => {
      const targetId = (header as HTMLElement).dataset.target;
      const content = targetId ? container.querySelector(`#${targetId}`) as HTMLElement | null : null;
      const icon = header.querySelector(".collapse-icon") as HTMLElement | null;
...
export function groupByNetwork(entries: ArpEntry[], type: "ipv4" | "ipv6"): Record<string, ArpEntry[]> {
  const groups: Record<string, ArpEntry[]> = {};
  ...
  const sortedGroups: Record<string, ArpEntry[]> = {};
...
export function filterEntries(entries: ArpEntry[], searchTerm: string): ArpEntry[] {
...
export async function viewLldpNeighbors(switchId: string): Promise<void> {
  ...
  const loadLldpData = async () => {
    const loadingEl = modal.querySelector("#lldp-loading") as HTMLElement | null;
    const contentEl = modal.querySelector("#lldp-content") as HTMLElement | null;
    const nameEl = modal.querySelector("#lldp-switch-name") as HTMLElement | null;
    const syncBtn = modal.querySelector("#sync-lldp-btn") as HTMLButtonElement | null;
    ...
    try {
      ...
      const result = await apiGet<LldpNeighbor[]>(`/api/switches/${switchId}/lldp-neighbors`);

      if (result.success) {
        const neighbors = (result.data as LldpNeighbor[]) || [];
        if (nameEl) nameEl.textContent = `- ${switchData.name} (${neighbors.length}${t("switch.entries") || "条"})`;
        ...
        if (contentEl) contentEl.innerHTML = renderLldpTable(neighbors);
        if (loadingEl) loadingEl.style.display = "none";
        if (contentEl) contentEl.style.display = "block";
      } else {
        if (loadingEl) loadingEl.innerHTML = `<p style="color: red;">${t("switch.load_lldp_failed") || "加载LLDP邻居失败"}: ${result.message || t("common.unknown_error") || "未知错误"}</p>`;
      }
    } catch (err) {
      const error = err as Error;
      if (loadingEl) loadingEl.innerHTML = `<p style="color: red;">${t("common.load_failed") || "加载失败"}: ${error.message}</p>`;
...
export function renderLldpTable(neighbors: LldpNeighbor[]): string {
  const isMac = (str: string): boolean => /^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$/.test(str);
...
export async function loadSwitchesForLldp(): Promise<void> {
  try {
    const result = await apiGet<Record<string, unknown>>("/api/switches?page_size=1000");
    if (result.success) {
      const data = result.data;
      const switches = (data as unknown as PagedData<Record<string, unknown>>)?.items || (Array.isArray(data) ? data : []);
      const select = elementCache.get("lldp-switch-select") as HTMLSelectElement | null;
      if (select) {
        select.innerHTML = `<option value="">${t("switch.select_switch") || "选择交换机..."}</option>` +
          switches.map((sw: Record<string, unknown>) => `<option value="${sw.id}">${sw.name}</option>`).join("");
      }
    }
  } catch (error) {
