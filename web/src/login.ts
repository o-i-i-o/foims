...
interface LoginDom {
  ...
  twoFactorForm: HTMLFormElement | null;
  tabs: NodeListOf<HTMLElement>;
  loginSections: NodeListOf<HTMLElement>;
  views: NodeListOf<HTMLElement>;
  loginView: HTMLElement | null;
  twoFactorView: HTMLElement | null;
  usernameInput: HTMLInputElement | null;
  passwordInput: HTMLInputElement | null;
  emailInput: HTMLInputElement | null;
  emailCodeInput: HTMLInputElement | null;
  sendLoginCodeBtn: HTMLButtonElement | null;
  rememberMeCheckbox: HTMLInputElement | null;
  submitBtn: HTMLButtonElement | null;
  errorMsg: HTMLElement | null;
  twoFactorCodeInput: HTMLInputElement | null;
  twoFactorSubmitBtn: HTMLButtonElement | null;
  backToLoginBtn: HTMLButtonElement | null;
  forgotPasswordLink: HTMLElement | null;
  forgotPasswordForm: HTMLFormElement | null;
  forgotPasswordError: HTMLElement | null;
  forgotPasswordSuccess: HTMLElement | null;
}

class LoginManager {
  private State: Record<string, LoginState> = {
    INIT: "init",
    SUBMITTING: "submitting",
    TWO_FACTOR: "two_factor",
  };

  private LoginMode: Record<string, LoginMode> = {
    PASSWORD: "password-login",
    EMAIL: "email-login",
  };

  private currentState: LoginState = this.State.INIT;
  private currentMode: LoginMode = this.LoginMode.PASSWORD;
  private tempAuthData: TempAuthData | null = null;
  private dom: LoginDom;

  constructor() {
    this.dom = {
      form: document.getElementById("login-form") as HTMLFormElement | null,
      twoFactorForm: document.getElementById("two-factor-form") as HTMLFormElement | null,
      ...
      emailInput: document.getElementById("email-login-input") as HTMLInputElement | null,
      emailCodeInput: document.getElementById("email-login-code") as HTMLInputElement | null,
      sendLoginCodeBtn: document.getElementById("send-login-code-btn") as HTMLButtonElement | null,
      rememberMeCheckbox: document.getElementById("remember-me") as HTMLInputElement | null,
      submitBtn: document.getElementById("login-submit-btn") as HTMLButtonElement | null,
      errorMsg: document.getElementById("login-error"),
      twoFactorCodeInput: document.getElementById("two-factor-code") as HTMLInputElement | null,
      twoFactorSubmitBtn: document.getElementById("two-factor-submit-btn") as HTMLButtonElement | null,
      backToLoginBtn: document.getElementById("back-to-login-btn") as HTMLButtonElement | null,
      forgotPasswordLink: document.getElementById("forgot-password-link"),
      forgotPasswordForm: document.getElementById("forgot-password-form") as HTMLFormElement | null,
  ...
  private updateFormValidation(): void {
    ...
    if (!usernameInput || !passwordInput || !emailInput || !emailCodeInput) return;
  ...
  private handleTabClick(targetBtn: HTMLElement): void {
    ...
    const mode = targetBtn.dataset.tab as LoginMode;
  ...
  private async submitPasswordLogin(username: string, password: string, rememberMe: boolean): Promise<void> {
  ...
  private async submitEmailLogin(email: string, code: string, rememberMe: boolean): Promise<void> {
    this.setLoading(true);
  ...
  private switchToTwoFactorView(): void {
  ...
  private async handleSendLoginCode(): Promise<void> {
    const email = this.dom.emailInput?.value.trim() ?? "";
    if (!email) {
  ...
  private startCountdown(btn: HTMLButtonElement, seconds: number): void {
  ...
  private setLoadingState(btn: HTMLButtonElement, isLoading: boolean): void {
    if (isLoading) {
  ...
  private formatErrorMessage(message?: string): string {
    ...
    if (message.includes("失败次数过多")) return t("login.too_many_attempts") || "登录失败次数过多，请5分钟后再试";
    if (message.includes("系统未初始化")) return t("login.system_not_initialized") || "系统未初始化，请联系管理员";
    if (message.includes("SMTP未配置")) return t("login.smtp_not_configured") || "系统邮件服务未配置，无法发送验证码";
  ...
  private async handleForgotPassword(e: Event): Promise<void> {
    ...
    if (!forgotPasswordError || !forgotPasswordSuccess || !forgotPasswordForm) return;
    ...
    try {
      ...
      if (err.message?.includes("Network")) msg = t("login.network_connection_failed") || "网络连接失败";
