...
import { showToast, renderTable, formatDateTime, debounce, handleError, appendPaginationToTable, escapeHtml, } from "../utils/ui.js";
...
function buildIPv4Content(network, networkIps) {
...
function buildIPv6Content(network, networkIps) {
...
function bindIPv4Events(modalContainer, networkId) {
...
function bindIPv6Events(modalContainer, networkId) {
...
export async function editNetworkType(id) {
    try {
        ...
        if (networkType) {
            openNetworkTypeModal(networkType);
        }
    }
    catch (error) {
        handleError(error);
...
export async function submitNetworkForm() {
    ...
    if (ipv4Dns === null && ipv4DnsStr && ipv4DnsStr.trim())
        return;
    if (ipv6Dns === null && ipv6DnsStr && ipv6DnsStr.trim())
        return;
    const networkData = {
        name: name.trim(),
