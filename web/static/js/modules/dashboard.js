  ...
  container.innerHTML = items.slice(0, 5).map(room => `
    <li>
      <div class="item-name">
        ...
        <div>
          <div>${room.name || '-'}</div>
          <div class="item-meta">${room.room_type || '-'}</div>
        </div>
      </div>
...
async function loadFallbackData() {
  try {
    const [networksResponse, regionsResponse, ipResponse] = await Promise.all([
      apiGet("/api/resources/networks"),
      apiGet("/api/resources/network-regions"),
      apiGet("/api/resources/ip")
    ]);

    const dashboardData = {
      networks: {
        networks: networksResponse.success ? networksResponse.data.length : 0,
        regions: regionsResponse.success ? regionsResponse.data.length : 0
      },
      ips: {
        total: ipResponse.success ? ipResponse.data.length : 0,
        active: ipResponse.success
          ? ipResponse.data.filter(({ status }) => status === "active").length
          : 0
      },
      activity: {
        operations_24h: 0
