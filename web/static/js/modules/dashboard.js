...
export async function loadDashboardData(forceRefresh = false) {
    ...
    const result = await safeAsync(() => apiGet("/api/system/dashboard-stats"), "加载仪表盘数据", { showToast: false });
...
async function fetchTopData(url) {
...
function initDashboardClickHandlers() {
    ...
    dashboard.addEventListener("click", function (e) {
        ...
        const card = target.closest(".stat-card.clickable, .dashboard-card");
        if (!card)
            return;
