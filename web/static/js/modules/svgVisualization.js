...
export class SVGVisualization {
    ...
    try {
      if (this.type === "workstation") {
        ...
        const [layoutResult, cabinetsResult] = await Promise.all([
          ...
          apiGet(`/api/resources/cabinets?page_size=1000`)
        ]);
  ...
  async autoDrawCabinetPositions(networkRegionId) {
    ...
    try {
      ...
      // 如果不支持，需要修改API或在前端过滤
      const cabinetsResult = await apiGet("/api/resources/cabinets?page_size=1000");
      ...
      showToast("绘制机位图失败: " + error.message, "error");
    }
  }
