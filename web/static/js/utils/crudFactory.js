...
export function createCrudManager(config) {
    const { endpoint, entityName, entityNameKey, loadListCallback, formId, modalId, validateCallback, transformDataCallback, afterCreateCallback, afterUpdateCallback, afterDeleteCallback, } = config;
    ...
    return {
        ...
        async create(data, options = {}) {
            ...
            if (result.success) {
                if (!silent)
                    showToast(t("common.create_success", { name: t(nameKey) }), "success");
                if (afterCreateCallback && result.data)
                    await afterCreateCallback(result.data);
                return { success: true, data: result.data };
            }
            else {
                if (!silent)
                    showToast(result.message || t("common.create_failed", { name: t(nameKey) }), "error");
        ...
        async update(id, data, options = {}) {
            ...
            if (result.success) {
                if (!silent)
                    showToast(t("common.update_success", { name: t(nameKey) }), "success");
                if (afterUpdateCallback && result.data)
                    await afterUpdateCallback(result.data);
                return { success: true, data: result.data };
            }
            else {
                if (!silent)
                    showToast(result.message || t("common.update_failed", { name: t(nameKey) }), "error");
                return { success: false, message: result.message };
            }
        },
        async delete(id, options = {}) {
            ...
            if (result.success) {
                if (!silent)
                    showToast(t("common.delete_success", { name: t(nameKey) }), "success");
                if (afterDeleteCallback)
                    await afterDeleteCallback(id);
                return { success: true };
            }
            else {
                if (!silent)
                    showToast(result.message || t("common.delete_failed", { name: t(nameKey) }), "error");
                return { success: false, message: result.message };
            }
        },
        openModal(title = "", data = null) {
            ...
            if (titleEl)
                titleEl.textContent = title || t(`${nameKey}.add`);
