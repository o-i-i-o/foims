...
function isValidIPv6(ip) {
    ...
    if (doubleColonCount > 1)
        return false;
    if (doubleColonCount === 1 && parts.length >= 8)
        return false;
...
export class IpConfigManager {
    ...
    async loadIps(ips) {
        ...
        try {
            ...
            if (this.config.loadNetworksByRegion) {
                ...
                regions = extractItems(regionsResult);
                ...
                if (regionIds.size === 0 && ips.some(ip => ip.network_id)) {
                    ...
                    const networkPromises = uniqueNetworkIds.map(id => apiGet(`/api/resources/networks/${id}`).catch(() => ({ success: false })));
            ...
            if (networkSelect && ipAddressInput) {
                ...
                if (networkId && ipAddress) {
                    ...
                    if (this.resourceType === "switch") {
                        ...
                        if (switchId)
                            ipData.parent_switch_id = switchId;
                        if (portId)
                            ipData.parent_port_id = portId;
                    }
    ...
    async bindRowEvents(row, networks, initialData) {
        ...
        if (initialData) {
            ...
            if (initialData.ip_address && ipInput)
                ipInput.value = initialData.ip_address;
            if (initialData.mac_address && macInput)
                macInput.value = initialData.mac_address;
