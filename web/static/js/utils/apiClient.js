...
export class ApiClient {
    ...
    static async request(url, options = {}, retryCount = 0) {
        ...
        const { skipAuthCheck, ...restOptions } = options;
        const mergedOptions = {
            ...defaultOptions,
            ...restOptions,
            headers: {
    ...
    static processQueue() {
        while (this.currentRequests < this.MAX_CONCURRENT_REQUESTS && this.requestQueue.length > 0) {
            ...
            if (queueItem) {
                this.processRequest(queueItem.url, queueItem.options, queueItem.retryCount, queueItem.requestKey).then(queueItem.resolve).catch((error) => {
    ...
    static isPublicAuthEndpoint(url) {
        ...
        return (normalizedUrl === "/api/auth/login" ||
